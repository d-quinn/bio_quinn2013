...
import mglobals
import helpers
import combining_SNPs_with_vcf as combine_w_vcf
...
def combine_snps():
    ...
    for sample in mglobals.samples_list:
        combine_w_vcf.combine_SNPs(input_vcf=mglobals.current_snp_file,
                                   orig_bam=join(mglobals.original_path, sample,
                                                 (sample + '_thout'), 'filter.bam'),
                                   new_bam=join(mglobals.alternate_path, sample,
                                                (sample + '_thout'), 'filter.bam'),
                                   output_f=('parental_alignment.csv'),
                                   cutoff_table=mglobals.coverage_cutoffs)
